...
const BlogCards = ({blogs,CurrentPage,selectedCategory,pagesize}) => {
    const filteredBlog=blogs.filter((blogs)=> !selectedCategory|| blogs.category===selectedCategory)
    .slice((CurrentPage-1)*pagesize,CurrentPage * pagesize);
    console.log(filteredBlog);
  return (
    <div className='grid lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-2 grid-cols-1  gap-8'> 
        {
            filteredBlog.map((blog)=><Link to={`/blogs/${blog.id}`} key={blog.id} className='p-5 shadow-lg rounded cursor-pointer'>
                <div>
